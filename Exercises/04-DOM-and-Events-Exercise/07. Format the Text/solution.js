function solve() {
  ...
  for (let i = 0; i < sentences.length; i += 3) {
    ...
    .map((p) => p.trimStart());
    let paragraphSentences = currentSentences.join('. ') + '.';
    let newParagraphElement = `<p>${paragraphSentences}</p>`;
    paragraphs.push(newParagraphElement);
  }
  divElement.innerHTML = paragraphs.join('');
}
